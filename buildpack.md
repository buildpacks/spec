...
An individual buildpack may only specify one process type with `default = true`. If multiple buildpacks define processes of the same type, the lifecycle MUST use the last process type definition ordered by buildpack execution for the combined process list. Therefore, a non-default process type may override a default process type, leaving the app image with no default. The lifecycle MUST select the last process type with `default = true` as the buildpack-provided default.
