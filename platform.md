...
```bash
/cnb/lifecycle/builder -buildpacks /cnb/by-id -group ./group.toml -plan ./plan.toml -project ./project.toml
```
...
- `-project` MAY specify input from a `project.toml` file path as defined in the [Data Format](#data-format) section.
...
## OCI Image
The OCI image containing the compiled application is generated by the Build Image during the [Build Phase](#build-phase).

If the `/cnb/lifecycle/builder` is invoked with the optional `-project` argument, the lifecycle MUST add a `"io.buildpacks.project"` Config label to the new OCI image with a [value as described in the data-format](#io.buildpacks.app.source-oci-image-label) section.
...
### io.buildpacks.project OCI Image label
